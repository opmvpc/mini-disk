...
void app_device_tick(void) {
    ...
    while (netmd_device_next_event(&app_device.thread, &event)) {
        ...
        switch (event.kind) {
            ...
            // T-043: the transfer view owns the run. The device panel only
            // passes them on; everything the burn means is decided there.
            case NetmdEvent_UploadProgress:
            ...
            case NetmdEvent_UploadError: {
                ...
                app_transfer_device_event(&event);
            } break;
...
String8 app_device_subtitle(void) {
    ...
    if (name.size == 0) { name = str8_lit("Net MD"); }
...
// --- the burn ---------------------------------------------------------------
// T-042 posted the plan from one button here; T-043 moved the whole thing into
// view_transfer.c, which owns the pre-flight, the run and the log. What is left
// is the one question the rest of the panel asks.
